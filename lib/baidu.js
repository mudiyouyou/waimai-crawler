...
function fetchPage(queryParam, context, pageNum) {
    ...
    var orderParam = {
        qt: 'orderlist',
        order_status: 9,
        start_time: queryParam.beginTime.format('YYYY-MM-DD'),
        end_time: queryParam.endTime.format('YYYY-MM-DD'),
        pay_type: 2,
        is_asap: 0
    };
    let orderOption = {
    ...
    return request.getAsync(getOrderURL + '&page=' + pageNum, orderOption).then(function (res) {
        return {orders: extractOrdersFromHtml(res.body), pageCount: 1};
    });
}
...
function postFetch(account, queryParam, orders) {
    ...
    let toConvertToCSV = _.partial(util.convertToCSV, account, queryParam, columns);
