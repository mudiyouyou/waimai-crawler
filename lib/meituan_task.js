...
class MeituanTask extends FetchTask {
    ...
    fetchPhone(token,order){
        logger.debug("fetch phone, order phone is " + order.recipient_phone);
        if(order.recipient_phone.search(/\*/)==-1){
            return order;
        }
        let requestJar = request.jar();
        token.cookies.split(';').forEach((v)=> {
            requestJar.setCookie(request.cookie(v), meituanUri);
        });
        let requestOption = {
            jar: requestJar,
            headers: {
        ...
        let phoneParam = {
            wmPoiId:order.wm_poi_id,
            wmOrderViewId:order.wm_order_id_view
        };
        let getPhoneUrl = "https://waimaie.meituan.com/v2/order/receive/processed/r/recipientPhone/v2?" + qs.stringify(phoneParam);
        //logger.debug(getPhoneUrl);
        return request.getAsync(getPhoneUrl, requestOption).then((res)=> {
            //logger.debug("fetch phone. response:" + JSON.stringify(res.body.data));
            if(res.body.data){
                order.recipient_phone = res.body.data.recipientPhone;
            }
            return order;
        });
    }
