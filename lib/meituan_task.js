...
class MeituanTask extends FetchTask {
    ...
    fetchPageAmount() {
        ...
        return request.getAsync(getOrderURL ,orderOption).then((res)=> {
            logger.debug("fetchPageAmount:" + res.body);
            if(!res.body) return promise.reject(new Error('Not found pageAmount'));
            let first = res.body.wmOrderList[0];
            return first.num;
        });
    }

    fetchPages(firstNum) {
        logger.debug("fetchPages start.");
        firstNum = firstNum + 1 - 10;
        let tasks = [];
        tasks.push(this.fetchPage(0));
        while(firstNum > 1){
            tasks.push(this.fetchPage(firstNum));
    ...
    fetchPage(firstNum) {
        logger.debug("fetchPage start.");
        let nextLabel="";
        if(firstNum!=0){
            nextLabel = {"day":this.option.beginTime.format('YYYYMMDD'),"page":0,"setPage":false,"setDay":true,"day_seq":firstNum,"setDay_seq":true};
        }
        let orderJar = request.jar();
        this.token.cookies.split(';').forEach((v)=> {
            orderJar.setCookie(request.cookie(v), meituanUri);
